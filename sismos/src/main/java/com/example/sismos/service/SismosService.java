...
public class SismosService {
	...
	/**
	 * Exponer endpoint para recibir 2 fechas, este se debe comunicar con un servicio que obtiene el detalle de los sismos entre los días pasados como parámetros.
	 * @param fechaInicio
	 * @param fechaTermino
	...
	/**
	 * Exponer endpoint que reciba dos magnitudes, el cual se debe comunicar con un servicio que obtiene el detalle de los sismos que se encuentran entre dichas magnitudes
	 * @param minMagnitude
	 * @param maxMagnitude
	...
	/**
	 * Exponer un endpoint que reciba 4 fechas, el cual se debe comunicar con un servicio que entregue los sismos ocurridos entre dichos rangos
	 * @param fechaInicioR1
	...
	/**
	 * Exponer endpoint para recibir 2 países y dos fechas, se debe comunicar con un servicio para contar la cantidad de sismos ocurridos en los datos que cumplan con los parámetros dados. 
	 * @param pais1
	 * @param pais2
	 * @param fecha1
	 * @param fecha2
	...
	public int getSismosByTwoDatesAndTwoCountries(String pais1, String pais2, String fecha1,String fecha2) {
		...
		List<Feature> sismos1 = sismos.getFeatures().parallelStream().filter(f -> f.getProperties().getPlace().contains(", " + pais1)).collect(Collectors.toList());
		List<Feature> sismos2 = sismos.getFeatures().parallelStream().filter(f -> f.getProperties().getPlace().contains(", " + pais2)).collect(Collectors.toList());
